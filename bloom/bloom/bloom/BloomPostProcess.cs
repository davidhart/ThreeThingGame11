...
namespace Effects
{
    class BloomPostProcess
    {
        ...
        public void LoadContent(GraphicsDevice device, ContentManager Content, PresentationParameters presentParams)
        {
            ...
            int screenWidth = presentParams.BackBufferWidth;
            int screenHeight = presentParams.BackBufferHeight;

            rt1 = new RenderTarget2D(device, screenWidth, screenHeight, false, presentParams.BackBufferFormat, presentParams.DepthStencilFormat, presentParams.MultiSampleCount, RenderTargetUsage.DiscardContents);
            rt2 = new RenderTarget2D(device, screenWidth, screenHeight, false, presentParams.BackBufferFormat, presentParams.DepthStencilFormat, presentParams.MultiSampleCount, RenderTargetUsage.DiscardContents);
        ...
        void DrawPass(RenderTarget2D target, Texture2D sampler0, Texture2D sampler1, Effect effect)
        {
            ...
            batch.Draw(sampler0, screenRect, Color.White);

            batch.End();
        }

        public void Draw(Texture2D bloomMap, Texture2D baseMap)
        {
            Draw(bloomMap, baseMap, DrawMode.Combine);
        }

        public void DrawDebugBloom(Texture2D bloomMap, Texture2D baseMap)
        {
            Draw(bloomMap, baseMap, DrawMode.Extract);
        }

        public void DrawDebugBlur(Texture2D bloomMap, Texture2D baseMap)
        {
            Draw(bloomMap, baseMap, DrawMode.Blur);
        }

        void Draw(Texture2D bloomMap, Texture2D baseMap, DrawMode mode)
        {
            ...
            DrawPass(null, rt1, baseMap, effectCombine);
        }
